...
| visual_scroll_direction | **PASS (vertical)** / horizontal confirmed at kernel level only | +v up / −v down visually correct on real content. Horizontal: kernel `REL_HWHEEL` signs verified by the automated probe (`+h left`, `−h right`); on-screen horizontal confirmation was blocked by an unrelated macOS defect (#62) — scroll bursts force-return the handoff before sustained observation |
