...
- `HELLO_ACK` advertises additive v1 capabilities. The current app rejects an
  ...
  matching helper.
- `InputSender` bounds and coalesces pointer movement and scroll (see
  ADR-0011), keeps keyboard and release paths independent, and cancels stale
  movement on local return.
