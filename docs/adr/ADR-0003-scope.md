...
2. **The CXI protocol is designed so both directions can be supported by adding message types only** (message type space separated without direction prefixes; types added later).
3. The reverse direction extension is only possible via the following paths:
...
4. **iPad is out of scope** (not technically impossible, but requires a separate design).
