...
if [ "$#" -ne 1 ]; then
    echo "usage: $0 redraw|cursor-rect|tracking-area|window-update|activation-control|status" >&2
    exit 2
fi

case "$1" in
    redraw|cursor-rect|tracking-area|window-update|activation-control|status) ;;
